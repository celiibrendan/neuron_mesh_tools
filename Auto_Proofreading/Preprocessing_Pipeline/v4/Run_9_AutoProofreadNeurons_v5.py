...
@schema
class AutoProofreadSynapse5(dj.Manual):
    definition="""
...
@schema
class AutoProofreadSynapseErrors5(dj.Manual):
    definition="""
    synapse_id           : bigint unsigned              # synapse index within the segmentation
    synapse_type: enum('presyn','postsyn')
    ver                  : decimal(6,2)                 # the version number of the materializaiton
...
@schema
class AutoProofreadStats5(dj.Manual):
    definition="""
...
@schema
class AutoProofreadNeurons5(dj.Computed):
    definition="""
    -> minnie.Decomposition()
    split_index          : tinyint unsigned             # the index of the neuron object that resulted AFTER THE SPLITTING ALGORITHM
    -> minnie.AutoProofreadVersion()             # the version of code used for this cell typing classification
    ...
    key_source = (minnie.Decomposition() & minnie.NucleiSegmentsRun4() 
             ...
             ) #& "segment_id=864691135517422218"#[dict(segment_id=k) for k in seg_ids_to_test]#& dict(segment_id = 864691135575137566) #864691135575137566
    ...
    def make(self,key):
        ...
        for sp_idx,split_index in enumerate(neuron_split_idxs):
            ...
            AutoProofreadSynapse5.insert(keys_to_write,skip_duplicates=True)
            
            keys_to_write_errors = AutoProofreadSynapseErrors_keys[sp_idx]
            AutoProofreadSynapseErrors5.insert(keys_to_write_errors,skip_duplicates=True)
        ...
        AutoProofreadStats5.insert(proofread_stats_entries,skip_duplicates=True)
...
curr_table = (minnie.schema.jobs & "table_name='__auto_proofread_neurons5'")
...
if not test_mode:
    AutoProofreadNeurons5.populate(reserve_jobs=True, suppress_errors=True, order="random")
else:
    AutoProofreadNeurons5.populate(reserve_jobs=True, suppress_errors=False, order="random")
print('Populate Done')
